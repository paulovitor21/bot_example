...
def download_files_from_gerp(desktop_bot: DesktopBot, orgs, latest_file):
    bot = ClassBot()
    ...
    try:
        ...
        # Converter para .xlsx após renomeação
        if renamed_file and renamed_file.endswith('.xls'):
            convert_xls_to_xlsx(renamed_file)
...
def convert_xls_to_xlsx(file_path):
    ...
    try:
        ...
        xls = pd.read_excel(file_path, engine='xlrd')
        # Define o novo nome para o arquivo .xlsx
        new_file_path = file_path.replace('.xls', '.xlsx')
        # Salva o arquivo convertido como .xlsx
        xls.to_excel(new_file_path, index=False, engine='openpyxl')
        print(f'Arquivo convertido e salvo como: {new_file_path}')
        return new_file_path
    except Exception as e:
        print(f"Erro ao converter {file_path} para .xlsx: {e}")
        return None
